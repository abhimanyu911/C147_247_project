...
@dataclass
class MagnitudeWarp:
    """Applies smoothly varying noise to all samples."

    Args:
        sigma (float): The scaling factor STD.
        knot (int): The complexity of the cubic spline curve.
        stack_dim (int): The dimension along which the left and right data
            are stacked. See ``ToTensor()``. (default: 1)
    """
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        orig_steps = np.arange(left.shape[0]) # time steps

        random_warps = np.random.normal(loc=1.0, scale=self.sigma, size=self.knot+2) # get the random values for each curve section/knot
        warp_steps = np.linspace(0, left.shape[0]-1., num=self.knot+2) # figure out which indices in the time step series marks each knot

        warper = np.array(CubicSpline(warp_steps, random_warps)(orig_steps)) # predict the cubic spline from the random values @ knot indices points and obtain the cubic spline values along the time series steps
        tensor_warper = torch.from_numpy(warper.reshape(-1,1)).to(torch.float32)

        left = torch.mul(tensor_warper, left) # broadcast across channels
        right = torch.mul(tensor_warper, right) # broadcast across channels
...
class LogSpectrogram:
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        #print(f"this is the log spectrogram output size!: {logspec.size()}")
